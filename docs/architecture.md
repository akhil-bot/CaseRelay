...
| Observability | Cloud Logging, Cloud Trace (Google-generated spans for MCP tool calls and Model Armor guardrail evaluations via Agent Gateway when `CASERELAY_PARTNER_MCP=1`; demonstrated end-to-end on 2026-08-31, trace `442a845a56a86c50ee5d35be1891cdd7`; ADK engine telemetry enabled) |
...
- **Agent Runtime** — eight reasoning engines in `us-central1` hosting the fleet. The checkpoint / sleep / deadline-triggered resume cycle around them is Firestore plus Pub/Sub push and Cloud Scheduler (hourly sweep at `0 * * * *`, dead-letter after 5 attempts, codified in `infra/bootstrap.sh`) rather than Agent Runtime itself: a Maya run ends on its checkpoints and a sweep restarts it, observed running unattended with nobody at the keyboard (the wake gap is up to an hour, depending on where in the cycle the checkpoint fell due)
