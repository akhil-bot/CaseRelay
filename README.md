...
| **Agent Observability** | Cloud Trace carries Google-generated spans for every MCP tool call and Model Armor guardrail evaluation that traverses the Agent Gateway. **Limitation:** ADK Agent Runtime does not export its own execution spans, so end-to-end tracing of agent reasoning is not achieved. |
