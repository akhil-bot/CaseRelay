...
| **Agent Observability** | Cloud Trace carries Google-generated spans for MCP tool calls and Model Armor guardrail evaluations that traverse Agent Gateway. Demonstrated end-to-end on 2026-08-31: trace `442a845a56a86c50ee5d35be1891cdd7` shows `MCP send tools/call family_status` as the root span with nested `apply_guardrail "Google Cloud Model Armor"` and `/mcp`. The fleet's default configuration routes partner calls through the in-process simulator (`CASERELAY_PARTNER_MCP=0`); setting the flag to `1` reproduces fresh gateway spans from partner calls (note: doing so currently regresses the Maya scenario — see [docs/deploy.md](docs/deploy.md) before enabling it). Agent Gateway remains active for all engine egress regardless of the MCP flag. **Limitation:** ADK Agent Runtime does not export its own execution spans, so end-to-end tracing of agent reasoning is not achieved. |
