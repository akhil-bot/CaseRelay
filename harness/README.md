...
the hardening plan. The plan itself is no longer tracked in `docs/`; `gate.py` is the surviving
statement of what each step had to achieve, and `run.sh` still points the agent at the plan by
path, so restore it from git history before running the loop.

Three files:

| File | What it is |
|---|---|
| `gate.py` | 38 executable acceptance checks. The definition of done. |
...
Before the first run, do the three items under "Before the loop runs" in `human/CHECKLIST.md` —
repository visibility is pass/fail for the submission, and none of them can be automated.
...
The plan was written against work that had been declared finished and was not: a day-17 wake that
was only a prompt, a Model Armor check that was a regex tuned to its own fixture, an audit writer
with no callers. An unattended loop whose stop condition is the agent's own judgement produces more
of that, faster.
...
| 1 · Real data | 3–8 | `t3.1`–`t3.3`, `t4.1`–`t4.4`, `t5.1`–`t5.3`, `t6.1`, `t7.1`–`t7.3`, `t8.1` |
| 2 · Control plane | 9–12 | `t9.1`–`t9.6`, `t10.1`–`t10.3`, `t11.1`–`t11.6`, `t12.1`–`t12.3` |
| 3 · Handover | 13–14 | `t13.1`, `t14.1` |

Step 1 is human-only, so it has no gate — see `human/CHECKLIST.md`.

Three gates need cloud access and are skipped without `--slow`: `t8.1` (end-to-end against the live
