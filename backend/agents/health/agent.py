...
def query_clinic(referral_id: str, case_id: str | None = None) -> dict:
    ...
    try:
        result = partners.clinic_status(referral_id, case_id=case_id)
    except TimeoutError:
        result = {"error": "timeout", "referral_id": referral_id, "note": "Clinic system did not respond within the allowed time."}
    if case_id:
        from backend.guards.commitment_guard import record_response
        record_response(case_id, "health", result)
    return result


def submit_appointment_status(case_id: str, status: str, summary: str) -> dict:
    refusal = workspace.set_commitment(case_id, "health", status)
    if refusal:
